# Chapter 315: Decision Transformer Trading

## Описание

Decision Transformer для sequence modeling подход к RL.

## Техническое задание

### Цели
1. Изучить теоретические основы метода
2. Реализовать базовую версию на Python
3. Создать оптимизированную версию на Rust
4. Протестировать на финансовых данных
5. Провести бэктестинг торговой стратегии

### Ключевые компоненты
- Теоретическое описание метода
- Python реализация
- Rust реализация для production
- Бэктестинг framework

### Метрики
- Accuracy / F1-score для классификации
- MSE / MAE для регрессии
- Sharpe Ratio / Sortino Ratio
- Maximum Drawdown

## Научные работы

1. **Decision Transformer: RL via Sequence Modeling**
   - URL: https://arxiv.org/abs/2106.01345
   - Год: 2021

## Структура
```
315_decision_transformer/
├── README.specify.md
├── docs/ru/
├── python/
└── rust/src/
```
