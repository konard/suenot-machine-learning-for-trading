# Chapter 318: Option Framework RL

## Описание

Options framework для hierarchical RL в trading.

## Техническое задание

### Цели
1. Изучить теоретические основы метода
2. Реализовать базовую версию на Python
3. Создать оптимизированную версию на Rust
4. Протестировать на финансовых данных
5. Провести бэктестинг торговой стратегии

### Ключевые компоненты
- Теоретическое описание метода
- Python реализация
- Rust реализация для production
- Бэктестинг framework

### Метрики
- Accuracy / F1-score для классификации
- MSE / MAE для регрессии
- Sharpe Ratio / Sortino Ratio
- Maximum Drawdown

## Научные работы

1. **Between MDPs and Semi-MDPs: A Framework for Temporal Abstraction in RL**
   - URL: https://arxiv.org/abs/1906.05862
   - Год: 2019

## Структура
```
318_option_framework_rl/
├── README.specify.md
├── docs/ru/
├── python/
└── rust/src/
```
