"""
Data loading and feature engineering for continual meta-learning trading.

This module provides:
- BybitClient for fetching cryptocurrency data
- FeatureGenerator for computing technical indicators
- SimulatedDataGenerator for testing with regime-changing data
"""
...
class SimulatedDataGenerator:
    """Generate simulated market data for testing continual learning scenarios."""
    ...
    @staticmethod
    def generate_sequential_regimes(
        klines_per_regime: int = 200,
        base_price: float = 50000.0
    ) -> List[Tuple[str, List[Kline]]]:
        """
        Generate labeled sequential market regimes for continual learning testing.

        Returns list of (regime_name, klines) tuples.
        """
        regimes = [
        ...
        for name, vol, trend in regimes:
            klines = SimulatedDataGenerator.generate_trending_klines(
                klines_per_regime, price, vol, trend
            )
            result.append((name, klines))
...
if __name__ == "__main__":
    print("Generating sequential regime data for continual learning...\n")
    regimes = SimulatedDataGenerator.generate_sequential_regimes(300)

    feature_gen = FeatureGenerator(window=20)

    for name, klines in regimes:
        features = feature_gen.compute_features(klines)
        print(f"Regime: {name}")
        print(f"  Klines: {len(klines)}, Features shape: {features.shape}")
        if len(features) > 0:
            print(f"  Feature means: {features.mean(axis=0)[:3].round(6)}")
        print()
