//! Trading strategy implementation using Continual MAML.

use crate::continual::algorithm::ContinualMAMLTrainer;
...
pub struct TradingStrategy {
    /// Continual MAML trainer for adaptation
...
impl TradingStrategy {
    ...
    pub fn new(
        ...
        Self {
            trainer: ContinualMAMLTrainer::new(
                model, inner_lr, outer_lr, inner_steps, first_order,
                ewc_lambda, replay_buffer_size,
            ),
    ...
    pub fn trainer(&self) -> &ContinualMAMLTrainer {
        &self.trainer
    }

    /// Get a mutable reference to the trainer
    pub fn trainer_mut(&mut self) -> &mut ContinualMAMLTrainer {
        &mut self.trainer
    }

    pub fn adaptation_window(&self) -> usize { self.adaptation_window }
    pub fn adaptation_steps(&self) -> usize { self.adaptation_steps }
    pub fn threshold(&self) -> f64 { self.threshold }
    pub fn strong_threshold(&self) -> f64 { self.strong_threshold }
    pub fn stop_loss(&self) -> f64 { self.stop_loss }
    pub fn take_profit(&self) -> f64 { self.take_profit }
}
...
mod tests {
    ...
    fn test_strategy_creation() {
        ...
        let strategy = TradingStrategy::new(model, 0.01, 0.001, 5, true, 100.0, 50)
    ...
    fn test_stop_loss_check() {
        ...
        assert!(strategy.check_stop_loss(100.0, 97.0, 1));
        assert!(!strategy.check_stop_loss(100.0, 99.0, 1));
        assert!(strategy.check_stop_loss(100.0, 103.0, -1));
        assert!(!strategy.check_stop_loss(100.0, 101.0, -1));
    ...
    fn test_take_profit_check() {
        ...
        assert!(strategy.check_take_profit(100.0, 105.0, 1));
        assert!(!strategy.check_take_profit(100.0, 102.0, 1));
        assert!(strategy.check_take_profit(100.0, 95.0, -1));
        assert!(!strategy.check_take_profit(100.0, 98.0, -1));
    }

    #[test]
    fn test_signal_generation() {
        let model = TradingModel::new(4, 8, 1);
        let strategy = TradingStrategy::new(model, 0.01, 0.001, 3, true, 100.0, 50);
