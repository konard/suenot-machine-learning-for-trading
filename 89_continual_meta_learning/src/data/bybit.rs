...
impl BybitClient {
    ...
    pub async fn fetch_klines(
        ...
        let response = self.client
            ...
            .send()
            .await
            .map_err(|e| CMLError::ApiError(format!("Request failed: {}", e)))?;

        let data: BybitResponse = response
            .json()
            .await
            .map_err(|e| CMLError::ApiError(format!("Failed to parse response: {}", e)))?;

        if data.ret_code != 0 {
            return Err(CMLError::ApiError(format!(
                "API error: {} - {}",
    ...
    pub async fn fetch_multi_symbol_klines(
        ...
        interval: &str,
        limit: usize,
    ) -> Result<Vec<(String, Vec<Kline>)>, CMLError> {
...
impl SimulatedDataGenerator {
    ...
    /// Generate sequential labeled regimes for continual learning testing
    pub fn generate_sequential_regimes(
        klines_per_regime: usize,
        base_price: f64,
    ) -> Vec<(&'static str, Vec<Kline>)> {
        ...
        for (name, vol, trend) in regimes {
            let klines = Self::generate_trending_klines(klines_per_regime, price, vol, trend);
            if let Some(last) = klines.last() {
                price = last.close;
            }
            result.push((name, klines));
        }
...
mod tests {
    ...
    #[test]
    fn test_sequential_regimes() {
        let regimes = SimulatedDataGenerator::generate_sequential_regimes(100, 50000.0);
