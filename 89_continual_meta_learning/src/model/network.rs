...
//!
//! This module provides a feedforward neural network suitable for
//! meta-learning with the MAML algorithm and continual learning.
...
impl DenseLayer {
    ...
    /// Forward pass with pre-activations for gradient computation
    pub fn forward_with_preactivations(&self, input: &[f64]) -> (Vec<f64>, Vec<f64>) {
        assert_eq!(input.len(), self.input_size, "Input size mismatch");
...
impl TradingModel {
    ...
    ///
    /// Uses 3 layers: input->hidden (ReLU), hidden->hidden (ReLU), hidden->output (Linear)
...
mod tests {
    ...
    fn test_gradient_computation() {
        let model = TradingModel::new(2, 4, 1);
        let features = vec![vec![0.1, 0.2], vec![0.3, 0.4]];
        let targets = vec![0.5, 0.6];
        let gradients = model.compute_gradients(&features, &targets, 1e-4);
        assert_eq!(gradients.len(), model.num_parameters());
    }
