...
impl BybitClient {
    ...
    pub fn get_ticker(&self, symbol: &str) -> Result<TickerInfo, BybitError> {
        ...
        response
            ...
            .into_iter()
            .next()
            .ok_or(BybitError::NoData)
    }

    /// Get multiple tickers at once
    pub fn get_all_tickers(&self) -> Result<Vec<TickerInfo>, BybitError> {
...
mod tests {
    ...
    fn test_interval_minutes() {
        ...
        assert_eq!(Interval::Day1.minutes(), 1440);
    }
    ...
    fn test_kline_true_range() {
        ...
        // Normal case: prev_close within range
        assert!((kline.true_range(100.0) - 15.0).abs() < 1e-10);

        // Gap up case
        assert!((kline.true_range(90.0) - 20.0).abs() < 1e-10);

        // Gap down case
        assert!((kline.true_range(120.0) - 25.0).abs() < 1e-10);
    }
}
