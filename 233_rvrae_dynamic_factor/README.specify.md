# Chapter 233: RVRAE Dynamic Factor

## Описание

Recurrent VAE для динамических факторных моделей.

## Техническое задание

### Цели
1. Изучить теоретические основы метода
2. Реализовать базовую версию на Python
3. Создать оптимизированную версию на Rust
4. Протестировать на финансовых данных
5. Провести бэктестинг торговой стратегии

### Ключевые компоненты
- Теоретическое описание метода
- Python реализация
- Rust реализация для production
- Бэктестинг framework

### Метрики
- Accuracy / F1-score для классификации
- MSE / MAE для регрессии
- Sharpe Ratio / Sortino Ratio
- Maximum Drawdown

## Научные работы

1. **RVRAE: Dynamic Factor Model Based on Variational Recurrent Autoencoder**
   - URL: https://arxiv.org/abs/2403.02500
   - Год: 2024

## Структура
```
233_rvrae_dynamic_factor/
├── README.specify.md
├── docs/ru/
├── python/
└── rust/src/
```
