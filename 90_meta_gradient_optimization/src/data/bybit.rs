...
//!
//! Provides async methods to fetch historical kline (candlestick) data
//! from the Bybit exchange API.

use chrono::{DateTime, Utc};
use serde::{Deserialize, Serialize};

use crate::MetaGradientError;
...
impl BybitClient {
    ...
    pub async fn fetch_klines(
        ...
        interval: &str,
        limit: usize,
    ) -> Result<Vec<Kline>, MetaGradientError> {
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| MetaGradientError::ApiError(format!("Request failed: {}", e)))?;

        let data: BybitResponse = response
            .json()
            .await
            .map_err(|e| MetaGradientError::ApiError(format!("Parse error: {}", e)))?;

        if data.ret_code != 0 {
            return Err(MetaGradientError::ApiError(format!(
                "API error: {} - {}",
    ...
    pub async fn fetch_multi_symbol_klines(
        ...
        interval: &str,
        limit: usize,
    ) -> Result<Vec<(String, Vec<Kline>)>, MetaGradientError> {
...
impl SimulatedDataGenerator {
    ...
    pub fn generate_klines(num_klines: usize, base_price: f64, volatility: f64) -> Vec<Kline> {
        ...
        let base_timestamp =
            chrono::Utc::now().timestamp_millis() - (num_klines as i64 * 3600000);
...
mod tests {
    ...
    #[test]
    fn test_regime_changing_data() {
        let klines = SimulatedDataGenerator::generate_regime_changing_klines(200, 50000.0);
