...
//!
//! Computes various technical indicators from price data for use in
//! meta-gradient optimized trading models.
...
impl FeatureGenerator {
    ...
    fn compute_volatility(&self, prices: &[f64]) -> Vec<f64> {
        ...
        returns[start..]
            ...
            .map(|w| {
                ...
                let variance =
                    w.iter().map(|x| (x - mean).powi(2)).sum::<f64>() / w.len() as f64;
                variance.sqrt()
            })
            .collect()
    }
    ...
    fn compute_rsi(&self, prices: &[f64]) -> Vec<f64> {
        ...
        for i in start..deltas.len() {
            ...
            let avg_gain: f64 =
                gains[window_start..=i].iter().sum::<f64>() / self.window as f64;
            let avg_loss: f64 =
                losses[window_start..=i].iter().sum::<f64>() / self.window as f64;
