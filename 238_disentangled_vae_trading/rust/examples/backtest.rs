//! Example: Backtest trading strategy with Disentangled VAE model

use disentangled_vae::{
    BacktestConfig, Backtester, DataLoader, DisentangledVAE, DisentangledVAEConfig,
    DisentanglementMethod, SignalGenerator,
};
use disentangled_vae::api::Kline;
use disentangled_vae::strategy::SignalGeneratorConfig;
use std::f64::consts::PI;

fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("Disentangled VAE Trading - Backtesting Example");
    println!("================================================\n");

    // Generate synthetic data with trend
    println!("Generating synthetic price data with trend...");
    let klines = generate_synthetic_klines(800, 0.0002);
    ...
    let config = DisentangledVAEConfig {
        ...
        output_dim: 1,
        beta: 4.0,
        method: DisentanglementMethod::BetaTCVAE,
        ..Default::default()
    };
    ...
    let result = backtester.run(&test_dataset.close_prices, &signals, 252 * 24);
