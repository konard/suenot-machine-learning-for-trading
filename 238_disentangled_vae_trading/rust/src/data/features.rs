//! Feature engineering for disentangled VAE
//!
//! Computes features including log returns, volatility, RSI, and volume changes
//! that serve as input to the VAE encoder.
...
impl Features {
    ...
    pub fn from_klines(klines: &[Kline], target_horizon: usize) -> Self {
        ...
        for i in 50..valid_len {
            ...
            // Volume change (log ratio vs 20-period average)
            ...
            values[[i - 50, 5]] = volume_change.clamp(-3.0, 3.0);
        ...
        for col in 0..self.values.ncols() {
            ...
            if std > 1e-8 {
                for row in 0..self.values.nrows() {
                    self.values[[row, col]] = (self.values[[row, col]] - m) / std;
                }
            }
        }
    }
...
mod tests {
    ...
    fn test_features_from_klines() {
        ...
        let features = Features::from_klines(&klines, 12);
    ...
    fn test_features_normalize() {
        let klines = create_test_klines(200);
        let mut features = Features::from_klines(&klines, 12);
        features.normalize();
