//! Dataset for disentangled VAE training and inference
...
mod tests {
    ...
    fn create_test_features(n: usize) -> Features {
        ...
        Features::from_klines(&klines, 12)
    }

    #[test]
    fn test_dataset_from_features() {
        let features = create_test_features(200);
        let dataset = Dataset::from_features(&features, 60);

        assert!(dataset.len() > 0);
        assert_eq!(dataset.inputs.dim().1, 60);
