...
impl BybitClient {
    ...
    pub async fn get_funding_rate(&self, symbol: &str) -> Result<FundingRate, BybitError> {
        ...
        let response: ApiResponse<FundingRateResult> = self.client.get(&url).send().await?.json().await?;
        ...
        let data = result.list.into_iter().next().ok_or_else(|| {
            BybitError::ParseError("No funding rate data returned".to_string())
        })?;

        let timestamp_ms: i64 = data.funding_rate_timestamp.parse().unwrap_or(0);
        let timestamp = Utc.timestamp_millis_opt(timestamp_ms).unwrap();

        Ok(FundingRate {
            timestamp,
            symbol: data.symbol,
            funding_rate: data.funding_rate.parse().unwrap_or(0.0),
            funding_interval: 8, // Default 8 hours for Bybit
        })
    }
