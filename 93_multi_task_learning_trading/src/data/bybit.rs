...
//! data from the Bybit exchange API for multi-task model training.
...
impl BybitClient {
    ...
    pub async fn fetch_klines(
        ...
        let response = self.client
            ...
            .send()
            .await
            .map_err(|e| MTLError::ApiError(format!("Request failed: {}", e)))?;

        let data: BybitResponse = response
            .json()
            .await
            .map_err(|e| MTLError::ApiError(format!("Failed to parse response: {}", e)))?;

        if data.ret_code != 0 {
            return Err(MTLError::ApiError(format!(
    ...
    pub async fn fetch_multi_symbol_klines(
        ...
        interval: &str,
        limit: usize,
    ) -> Result<Vec<(String, Vec<Kline>)>, MTLError> {
...
impl SimulatedDataGenerator {
    ...
    /// Generate klines with regime changes for multi-task testing.
